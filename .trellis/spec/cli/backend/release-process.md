...
|---|---|---|
| `trellis-ivy` | User-facing CLI | GitHub Actions only |
| `trellis-ivy-core` | Programmatic core APIs used by the CLI and external integrations | GitHub Actions only |
...
2. Verify public npm visibility:
   ```bash
   npm view trellis-ivy@<version> version dist-tags --json --registry=https://registry.npmjs.org/
   npm view trellis-ivy-core@<version> version dist-tags --json --registry=https://registry.npmjs.org/
   ```
...
| Packed dependency | Published CLI package must depend on `trellis-ivy-core` with the exact release version. |
...
8. publish `trellis-ivy-core` if missing
9. publish `trellis-ivy` if missing
