...
|---|---|---|
| `@liushuang/trellis` | User-facing CLI | GitHub Actions only |
| `@liushuang/trellis-core` | Programmatic core APIs used by the CLI and external integrations | GitHub Actions only |
...
2. Verify public npm visibility:
   ```bash
   npm view @liushuang/trellis@<version> version dist-tags --json --registry=https://registry.npmjs.org/
   npm view @liushuang/trellis-core@<version> version dist-tags --json --registry=https://registry.npmjs.org/
   ```
...
| Packed dependency | Published CLI package must depend on `@liushuang/trellis-core` with the exact release version. |
...
8. publish `@liushuang/trellis-core` if missing
9. publish `@liushuang/trellis` if missing
