...
设计并后续实现 `trellis-ivy-core`，把 Trellis CLI 里已经成型的 channel/task 领域逻辑抽成可发布的 TypeScript core package。CLI 继续作为用户命令入口，但不再独占 channel 事件、thread reducer、storage、watch、task record 等核心语义。外部 Node 消费方后续应能通过 in-process API 调用同一套 core，而不是 subprocess 调 `trellis channel ...`。
...
- 新增 workspace package：`packages/core`，npm 包名 `trellis-ivy-core`。
- `packages/cli` 依赖 `trellis-ivy-core@workspace:*`。
