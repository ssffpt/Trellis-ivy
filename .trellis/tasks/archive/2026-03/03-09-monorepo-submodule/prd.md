...
- npm 发布为 `trellis-ivy`，`files: ["dist", "bin", "README.md", "LICENSE"]`
...
包名 `trellis-ivy`，当前从仓库根发布。搬到 `packages/cli/` 后：

| 项 | 变化 |
|---|---|
| `bin`, `main`, `types` 字段 | **不用改** — 相对于 package.json 的路径不变 |
| `files` 字段 | 需确认 `README.md` 和 `LICENSE` 的位置（可在 cli/ 放精简版或脚本拷贝） |
| CI publish.yml | 加 `working-directory: packages/cli` 或 `pnpm --filter trellis-ivy publish` |
