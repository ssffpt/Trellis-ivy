...
- **简单 UI 改动**（如调整文案、修改颜色、单个元素状态变更）：使用下方"UI 描述格式"中的结构化描述（表格+状态机），无需生成原型。
- **不确定改动复杂度时**：询问用户"此任务涉及 UI 改动，建议先做一个 HTML 原型确认视觉方向，您看可以吗？"。用户同意后生成原型，用户拒绝则用结构化表格描述。
...
5. 用户确认方案后，**涉及 UI 改动时，若已生成原型并获用户确认，必须将关键视觉和交互决策以结构化格式写回 `prd.md`。** HTML 原型本身不作为实现依据，仅 `prd.md` 中的结构化描述才是 AI 编码时的参考。
6. 提出剩余问题中最有价值的一个。
7. 在问题中包含你的推荐答案。
8. 每次用户回答后，更新 `prd.md`，然后继续。
9. **PRD 门禁审核**：当 `prd.md` 中无 TODO/占位符、且所有已知问题都已讨论完毕时，AI 主动提议"prd.md 看起来已完整，是否启动门禁审核？"。用户同意后，按以下流程执行：
