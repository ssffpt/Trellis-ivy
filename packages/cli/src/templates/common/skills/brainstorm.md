# Trellis 头脑风暴

## 不可妥协的访谈契约

就这个计划的每个方面对我进行不懈的追问，直到我们达成共同的理解。沿着设计树的每个分支逐一排查，逐个解决决策之间的依赖关系。对于每个问题，给出你的推荐答案。

每次只问一个问题。

## 不可妥协的证据规则

如果某个问题可以通过探索代码库来回答，那就直接去探索代码库。

这是强制性的。在向用户提问之前，先检查答案是否已经存在于代码、测试、配置、文档、现有规范或任务历史中。

不要请用户确认代码库能回答的事实。只询问关于产品意图、偏好、范围、风险承受能力或在检查后仍然不明确的事项。

---

在第一阶段规划期间使用此技能，将用户的请求转化为清晰的需求和规划产物。

## 前置条件

仅在已获得任务创建同意且用户准备好进入 Trellis 规划时使用此技能。

如果还没有任务，创建一个：

```bash
TASK_DIR=$({{PYTHON_CMD}} ./.trellis/scripts/task.py create "<简短的任务标题>" --slug <slug>)
```

使用用户请求中的简洁标题。使用不带日期前缀的 slug。`task.py create` 会自动添加 `MM-DD-` 目录前缀。

`task.py create` 会创建默认的 `prd.md`。在提出后续问题之前，先用当前的理解更新该文件。

## 反模式："这个太简单了，不需要设计"

每个项目都要经过规划流程。一个配置变更、一个单函数工具、一个待办列表——全都需要。

"简单"的项目恰恰是未经检验的假设造成最多浪费的地方。设计可以很简短（几句话就够了），但你必须先展示出来并获得用户批准。

## UI 改动处理策略

- **简单 UI 改动**（如调整文案、修改颜色、按钮状态变更）：使用下方"UI 描述格式"中的结构化描述（表格+状态机），无需生成原型。
- **复杂 UI 改动**（新增页面、复杂布局调整、交互流程变更）：AI 应主动提议用户"此任务涉及 UI 改动，建议先做一个 HTML 原型确认视觉方向，您看可以吗？"。用户同意后生成原型，用户拒绝则用结构化表格描述。

原型保存路径：`$TASK_DIR/prototype.html`（与 `prd.md` 同目录）。原型使用纯 HTML + Tailwind CDN，独立于项目技术栈，仅作一次性视觉参考。

---

<HARD-GATE>
在你展示设计方案并获得用户明确批准之前，不要调用任何实现技能、编写任何代码、创建任何文件或采取任何实现行动。
这适用于所有项目，无论看起来多简单。
</HARD-GATE>

---

## 规划流程

1. 在 `prd.md` 中记录用户的请求和初始已知事实。
2. 在提问之前先检查可用证据：
   - 代码、测试、夹具和配置
   - README 文件、文档、现有规范和领域说明
   - 如有，相关的 Trellis 任务、研究文件和会话历史
3. 将发现的内容分为：
   - 已确认的事实
   - 仍需要从用户处了解的产品意图
   - 仍需要用户决定的范围或风险决策
   - 可能超出范围的事项
4. **提出 2-3 种方案。** 在展示单一设计之前，先探索不同的实现路径。对每种方案说明：
   - 方案的核心思路
   - 优点和缺点
   - 你的推荐及理由
   以对话方式展示选项，先展示你推荐的方案并解释原因。
5. **涉及 UI 改动时，若已生成原型并获用户确认，必须将关键视觉和交互决策以结构化格式写回 `prd.md`。** HTML 原型本身不作为实现依据，仅 `prd.md` 中的结构化描述才是 AI 编码时的参考。
6. 提出剩余问题中最有价值的一个。
7. 在问题中包含你的推荐答案。
8. 每次用户回答后，更新 `prd.md`，然后继续。
9. **PRD 门禁审核**：当用户表示 prd.md 已就绪（如"审核吧"、"检查一下 prd"、"门禁"）时，
调度 `trellis-review` agent 审核当前任务目录下的 prd.md。审核流程如下：

   1. 调用 `trellis-review` agent，传入任务目录路径
   2. 审核 agent 输出问题清单到 `$TASK_DIR/review.md`（只审不改）
   3. 若存在 C/H/M 级问题，由当前会话的规划 agent 修复 prd.md
   4. 修复后再次调度 `trellis-review` 重新审核
   5. **最多 3 轮**。第 3 轮仍有 C/H/M → 暂停并请用户决策

   用户只看最终审核结果，无需参与中间过程。

   **门禁通过后**，审核 agent 会判定任务级别（轻量/标准）并写入 `review.md`。
   若为标准任务，审核结果会包含"建议生成 design.md"的提示。用户确认后，继续产出 design.md 并等待用户批准。

不要编造项目特定的产品/规范层级。如果仓库中已有产品、领域或规范文档，请使用它们。如果没有，则基于现有证据继续。

## 提问规则

每次消息只问一个问题。

每个问题必须包含：

- 需要做出的决定
- 为什么这个答案很重要
- 你的推荐答案
- 如果用户选择不同方案的权衡

不要问流程性问题，例如是否搜索、检查文件或继续头脑风暴。直接完成证据工作。仅在剩余问题是关于产品决策、偏好、范围边界或风险承受能力选择时才询问用户。

## 产物规则

### 文件结构

轻量任务只需 `prd.md`。标准任务需要两个文件：

```
prd.md      — 需求、验收标准（"做什么"）
design.md   — 技术设计（"怎么设计"）
```

### 任务分级标准

**轻量任务 → prd.md only（~80%）**：默认所有任务。

**标准任务 → prd.md + design.md（~20%）**：满足任一即触发：
1. 新增外部依赖（项目中不存在的包/服务/API）
2. 新增数据表（CREATE TABLE / 新增 model）
3. 新增顶层模块/包（新建 packages/ 或 src/ 子目录）
4. 变更认证/鉴权机制（修改 login/verify/permission 核心逻辑）
5. 替换技术方案（同一功能换实现机制：sync→async、轮询→推送、更换核心算法）

不满足以上条件默认判为轻量级，不写 design.md。

### prd.md 结构

记录需求、验收标准：

```markdown
# <Task Title>

## 目标

<why + what>

## 需求

* ...

## 验收标准

* [ ] ...

## 超出范围

* ...

## 研究参考

* <links to research/*.md or external references>
```

### design.md 结构（标准任务）

记录技术设计，按小节组织，每个小节有独立上限：

```markdown
# <Task Title> - 技术设计

## 架构边界（≤15 行）
哪些模块/层负责什么，职责划分。

## 关键数据流（≤15 行）
模块间传递什么数据、谁调用谁（不写签名）。

## 兼容性与迁移（≤10 行）
向前/向后兼容、迁移路径、回滚策略。

## 权衡与选择理由（≤10 行）
为什么选 A 不选 B，放弃了什么。
```

**各小节可以省略**：轻量复杂任务可能只需要"架构边界"和"权衡"，不需要四个都写。

**禁止写的内容（适用于所有小节）：**
- 伪代码、函数签名、类型定义
- 实现步骤、文件级改动清单
- 代码片段或配置样例
- 任何"怎么做"的细节

**硬性限制：** 总计不超过 50 行。如果某个小节写满了还在继续，说明任务应该拆成子任务，而不是在设计里堆更多内容。

### review.md 结构

记录门禁审核结果和修复历史：

```markdown
# <Task Title> - 审核记录

## 审核维度

| 维度 | 状态 | 备注 |
|------|------|------|
| 需求完整性 | ✅/⚠️/❌ | |
| 技术可行性 | ✅/⚠️/❌ | |
| 范围合理性 | ✅/⚠️/❌ | |
| 验收标准可测性 | ✅/⚠️/❌ | |

## 修复记录

- <每轮修复的内容>

## 最终结论

- <通过/不通过>

## 遗留问题

- <如有>
```

## UI 描述格式

涉及界面改动时，用以下结构描述，不要只用自然语言：

### 布局网格
| 区域 | 内容 | 宽度 | 对齐 |
|---|---|---|---|

### 交互状态
| 状态 | 触发条件 | 效果 |
|---|---|---|

### 间距规范
- 卡片间距：16px
- 内边距：24px
- 圆角：8px

## 质量门槛

在宣布规划完成之前：

- `prd.md` 包含可测试的验收标准。
- 代码库可回答的问题已通过检查得到答案。
- 剩余的开放问题确实是关于用户意图或范围的。
- 标准任务的 `design.md` 总计不超过 50 行，按小节结构化，且不包含实现细节。
- **PRD 已通过 `trellis-review` 门禁审核（`review.md` 最终状态为 ✅）。**
- **已提出 2-3 种方案并让用户做出选择。**
- **用户已明确批准规划产物。**
- **涉及 UI 改动时，原型已确认且关键视觉/交互参数已同步到 `prd.md`。**

在用户明确批准之前，不要开始实现。如果用户说"开始吧"、"就这么做"、"ok"或类似的话，视为明确批准。不要用"默认继续"来替代明确批准。

## 规格自检

在最终提交规划产物之前，以全新的视角审视它们：

1. **占位符扫描：** 有没有"待定"、TODO、未完成的章节或模糊的需求？修复它们。
2. **内部一致性：** 各章节之间有矛盾吗？`prd.md` 与 `design.md` 之间是否一致？
3. **范围检查：** 这是否聚焦到可以用一个实现计划覆盖，还是需要进一步拆分？
4. **模糊性检查：** 有没有需求可以被两种方式理解？如果有，选择一种并明确写出来。
5. **设计越界检查：** `design.md` 各小节是否超出独立上限？是否包含伪代码、实现步骤、代码片段？如果有，删掉，回退到"做什么+为什么"。如果某个小节持续超限，说明任务需要拆分子任务。

发现问题就直接内联修复。修好继续推进。
