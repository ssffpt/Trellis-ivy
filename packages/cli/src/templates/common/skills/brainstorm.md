...
9. **PRD 门禁审核（仅复杂任务）**：判断任务分类——轻量级任务（预计仅需 `prd.md`）可跳过门禁，直接让用户确认；复杂任务（需要 `design.md`）在 `prd.md` 内容稳定后，调度 `trellis-review` agent 对 `prd.md` 进行独立审核。审核不通过时 AI 自动修复并重新审核（最多 2 轮）。审核通过后继续。
...
轻量级任务（仅需 `prd.md`）可跳过步骤 9 和 10-11 的门禁审核，直接进入用户确认。
...
- **复杂任务的 PRD 已通过 `trellis-review` agent 门禁审核（`review.md` 最终状态为 ✅）；轻量级任务已获用户快速确认。**
