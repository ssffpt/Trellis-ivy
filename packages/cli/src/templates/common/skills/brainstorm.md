...
轻量任务只需 `prd.md`。复杂任务需要三个文件：

```
prd.md      — 需求、验收标准（"做什么"）
design.md   — 技术设计（"怎么设计"）
implement.md — 执行计划（"怎么做"）
```
...
不满足以上条件默认判为轻量级，不写 design.md 和 implement.md。
...
`implement.md` 是人类可读的执行计划，不替代 `implement.jsonl`（后者仅列出 spec/research 清单）。
...
- 复杂任务的 `design.md` 总计不超过 50 行，按小节结构化，且不包含实现细节。
- 复杂任务的 `implement.md` 包含有序的实现清单、验证命令和审查关口。
...
2. **内部一致性：** 各章节之间有矛盾吗？`prd.md`、`design.md`、`implement.md` 之间是否一致？
...
6. **执行计划检查：** `implement.md` 的步骤是否有序？验证命令是否可执行？审查关口是否明确？
