...
9. **PRD 门禁审核**：当 `prd.md` 中无 TODO/占位符、且所有已知问题都已讨论完毕时，AI 主动提议"prd.md 看起来已完整，是否启动门禁审核？"。用户回复"好"、"ok"、"审核"、"可以"、"开始吧"或类似肯定表达时触发。
   ...
   2. 若审核通过（C = 0, H = 0），进入步骤 10。
   3. 若存在 C/H/M 级问题，修复 `prd.md` 后再次调度审核，**最多自动执行 3 轮**。
   4. 第 3 轮仍未通过时，暂停自动流程，向用户展示剩余问题，由用户决定：继续修复、缩小范围、或手动标记通过。

   用户只看最终审核结果，无需参与中间过程。

10. **请求实现批准**：门禁通过后，AI 向用户展示审核结论（`review.md` C = 0, H = 0）并提议"规划已完成，是否开始实现？"。用户回复"ok"、"好"、"开始"、"可以"或类似肯定表达后进入实现阶段。
...
- **PRD 已通过 `trellis-review` 门禁审核（`review.md` 汇总中 C = 0 且 H = 0）。**
- **已提出 2-3 种方案并让用户做出选择。**
- **用户已明确批准规划产物。**
