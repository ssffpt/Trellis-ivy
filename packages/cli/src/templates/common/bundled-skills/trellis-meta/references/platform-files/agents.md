...
| `trellis-implement` | Implement against `prd.md`, optional `design.md`, `implement.jsonl`, and related spec/research. |
...
2. **Specify the read order**. Agents must know to start from the active task, read jsonl/spec context, then read `prd.md`, `design.md` if present.
