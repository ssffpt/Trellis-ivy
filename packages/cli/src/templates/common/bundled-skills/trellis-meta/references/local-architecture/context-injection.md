...
| task context | `.trellis/tasks/<task>/prd.md`, `design.md`, `research/` | Current task requirements, design, and research. |
...
1. **hook push**: a platform hook injects jsonl-referenced files plus `prd.md`, `design.md` if present before the agent starts.
...
In both modes, JSONL files in the task directory are the manifest for spec/research context. Task artifacts are read separately in this order: `prd.md` -> `design.md if present`.
