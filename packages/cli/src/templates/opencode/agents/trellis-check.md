...
- **如果标记缺失**：hook 注入未触发（Windows + Claude Code、`--continue` 恢复、fork 发行版、hooks 被禁用等情况）。从调度提示的第一行 `Active task: <path>` 获取任务路径（备选方案：运行 `python3 ./.trellis/scripts/task.py current --source`），然后依次读取 `<task-path>/check.jsonl`、其中列出的各文件、`<task-path>/prd.md`，再开始工作。
