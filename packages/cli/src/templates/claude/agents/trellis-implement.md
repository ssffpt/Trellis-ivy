...
- **如果标记不存在**：hook 注入未触发（Windows + Claude Code、`--continue` 恢复、fork 分发、hooks 禁用等）。从派发提示的第一行 `Active task: <path>` 找到活动任务路径，然后读取 `<task-path>/implement.jsonl`、每个列出的文件、`<task-path>/prd.md`、`<task-path>/design.md`（如果存在）和 `<task-path>/implement.md`（如果存在），然后再开始工作。
...
2. **理解任务产物** - 读取 prd.md、design.md（如果存在）和 implement.md（如果存在）
...
读取任务的 prd.md、design.md（如果存在）和 implement.md（如果存在）：
