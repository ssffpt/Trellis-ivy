...
> = {
  "trellis-review": {
    claude: {
      description:
        "PRD/Design 门禁审核 Agent。独立审核需求文档和技术设计的质量，输出清单式审核报告。只审不改，禁止修改 prd.md / design.md。",
...
3. Read the task's \`prd.md\` (requirements), then \`design.md\` if present (technical design).

If \`${jsonl}\` has no curated entries (only a seed row, or the file is missing), fall back to: read the task artifacts, list available specs with \`python3 ./.trellis/scripts/get_context.py --mode packages\`, and pick the specs that match the task domain yourself. Do NOT block on the missing jsonl — lightweight tasks may be PRD-only, while standard tasks may also include \`design.md\`.
